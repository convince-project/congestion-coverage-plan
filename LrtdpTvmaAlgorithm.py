...
def LrtdpTvmaAlgorithm():
    ...
    def lrtdp_tvma(self):
        while not(self.mdp.visited(self.vinitGlobal) and not self.mdp.visited(self.vinitGlobal) is None) and self.tGlobal < self.timeBoundGlobal:
            self.lrtdp_tvma_trial(self.vinitGlobal, self.convergenceThresholdGlobal, self.timeBoundGlobal)
    ...
    def lrtdp_tvma_trial(self, vinitparameter, thetaparameter, maxtimeparameter):
            ...
            while not visited.isEmpty():
                (v, t) = visited.pop()
                if not self.checkSolved(v, t, thetaparameter):
                    break
